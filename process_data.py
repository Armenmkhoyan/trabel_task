...
def process_segment(segment_sql_path: str):
    ...
    insert_user_id_into_temp_segment = load_sql_file("queries/insert_user_id_into_temp.sql")
    for date in action_days:
        formatted_insert_into_sampled_activity_segment = insert_into_sampled_activity_segment.format(date=date)
        formatted_insert_user_id_into_temp_segment = insert_user_id_into_temp_segment.format(date=date)
        execute_sql_query(formatted_insert_into_sampled_activity_segment)
        logging.info("Inserting of 2000 users into sampled_activity_data successfully done.")
