...
from patterns import start_date, end_date
from db_setup import create_db, create_tables, execute_sql_query, create_partition_on_user_action
from generate_data import generate_users, generate_event
...
def process_all_users():
    ...
    logging.info(f"Inserting of {total_users} records into users table  took a {(end / 60):.2f} minutes")
...
def process_all_events():
    ...
    while day_count <= events_day_limit:
        ...
        date_representation = time.strftime("%Y-%m-%d", time.localtime(action_start_timestamp))
    ...
    logging.info(f"Inserting of 10000000 records into user_activity table took a  {(end / 60):.2f} minutes")
